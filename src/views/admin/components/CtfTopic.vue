 ...
 * @Date: 2022-08-02 15:41:53
 * @LastEditors: QKSword 1064615639@qq.com
 * @LastEditTime: 2022-12-15 17:10:40
 * @FilePath: \lab-front\src\views\admin\components\CtfTopic.vue
 * @Description: 管理ctf题目组件
-->
...
<template>
  <div class="components-container">
    <div class="ctfTopic">
      <div class="filter-container">
        <div class="filter-group">
          ...
          <el-select
            ...
            @change="topicSelect"
          >
            <el-option v-for="option in filter.ctfTypeOptions" :key="option.value" :label="option.label" :value="option.value" />
          </el-select>
        </div>
        <div class="filter-group">
      ...
      <operate-ctf-topic v-model="topicDialog" :topic-info="topicInfo" @topicResult="getTopicResult" />
    </div>
  </div>
</template>
...
import { getTopicInfo, deleteTopic } from '@/api/admin/sys-ctf'
import OperateCtfTopic from '@/views/admin/components/OperateCtfTopic.vue'

var proto = require('~/proto.js')

export default {
  components: { OperateCtfTopic },
  data() {
    return {
      loading: false,
      // feat(QKSword)：显示数据
  ...
  methods: {
    ...
    clickCreateTopic() {
      ...
      newTopicInfo.title = ''
      newTopicInfo.desc = ''
      newTopicInfo.score = 0
      newTopicInfo.time = 1
      newTopicInfo.overTime = 5
      newTopicInfo.url = ''
      newTopicInfo.image = ''
      newTopicInfo.flag = 'Flag{XXXXXX}'
      this.topicInfo = newTopicInfo

      newTopicDialog.topicType = this.filter.ctfType.toUpperCase()
    ...
    createDeleteTopicProto(deleteTopicId) {
      var sysCtfDeleteTopic = proto.lookup('SysCtfDeleteTopic')
      var msg = sysCtfDeleteTopic.create({
        type: this.filter.ctfType,
        id: deleteTopicId
      })

      var encodeMsg = sysCtfDeleteTopic.encode(msg).finish()
      return encodeMsg.slice(0, encodeMsg.length)
    },
    // feat(QKSword):生成getTopicNum的protobuf
    createGetTopicNumProto() {
      var sysCtfGetTopicNum = proto.lookup('SysCtfGetTopicNum')
      var msg = sysCtfGetTopicNum.create({
        type: this.filter.ctfType
      })

      var encodeMsg = sysCtfGetTopicNum.encode(msg).finish()
      return encodeMsg.slice(0, encodeMsg.length)
    },
    ...
    getPageTopicInfo() {
      ...
      getTopicInfo(this.createGetTopicInfoProto()).then(response => {
        ...
        var data = response.data
        if (data.error === '') {
          this.pagination.totalPage = data.sysCtfTopicNum
          this.tableData.display = data.sysCtfTopicInfo
        } else {
          this.$message({
            message: data.error,
            type: 'error',
    ...
    createGetTopicInfoProto() {
      var sysCtfGetTopicInfo = proto.lookup('SysCtfGetTopicInfo')
      var msg = sysCtfGetTopicInfo.create({
        type: this.filter.ctfType,
        pageSize: this.pagination.pageSize,
        currentPage: this.pagination.currentPage
      })

      var encodeMsg = sysCtfGetTopicInfo.encode(msg).finish()
      return encodeMsg.slice(0, encodeMsg.length)
    },
