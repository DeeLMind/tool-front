<template>
  <div class="components-container">
    <div class="labsTopic">
      <div class="filter-container">
        <div class="filter-group">
          ...
          <el-select
            ...
            @change="topicSelect"
          >
            <el-option v-for="option in filter.labsTypeOptions" :key="option.value" :label="option.label" :value="option.value" />
          </el-select>
        </div>
        <div class="filter-group">
      ...
      <operate-labs-topic v-model="topicDialog" :topic-info="topicInfo" @topicResult="getTopicResult" />
    </div>
  </div>
</template>
...
import { getTopicInfo, deleteTopic } from '@/api/admin/sys-labs'
import OperateLabsTopic from './OperateLabsTopic.vue'

var proto = require('~/proto.js')

export default {
  components: { OperateLabsTopic },
  data() {
    return {
      loading: false,
      // feat(QKSword)：显示数据
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      newTopicDialog.topicType = this.filter.labsType.toUpperCase()
    ...
    getPageTopicInfo() {
      ...
      getTopicInfo(this.createGetTopicInfoProto()).then(response => {
        ...
        if (data.error === '') {
          this.pagination.totalPage = data.sysLabsTopicNum
          this.tableData.display = data.sysLabsTopicInfo
        } else {
          this.$message({
            message: data.error,
            type: 'error',
    ...
    createGetTopicInfoProto() {
      var sysLabsGetTopicInfo = proto.lookup('SysLabsGetTopicInfo')
      var msg = sysLabsGetTopicInfo.create({
        type: this.filter.labsType,
        pageSize: this.pagination.pageSize,
        currentPage: this.pagination.currentPage
      })

      var encodeMsg = sysLabsGetTopicInfo.encode(msg).finish()
    ...
    createDeleteTopicProto(deleteTopicId) {
      var sysLabsDeleteTopic = proto.lookup('SysLabsDeleteTopic')
      var msg = sysLabsDeleteTopic.create({
        type: this.filter.labsType,
        id: deleteTopicId
      })

      var encodeMsg = sysLabsDeleteTopic.encode(msg).finish()
      return encodeMsg.slice(0, encodeMsg.length)
    },
