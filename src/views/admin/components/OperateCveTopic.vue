 ...
 * @Date: 2022-12-15 16:24:17
 * @LastEditors: QKSword 1064615639@qq.com
 * @LastEditTime: 2022-12-16 15:57:38
 * @FilePath: \lab-front\src\views\admin\components\OperateCveTopic.vue
 * @Description: 操作cve题目组件
-->
<template>
  <el-dialog
...
<script>
import { createTopic, updateTopic } from '@/api/admin/sys-cve'
  ...
  methods: {
    ...
    clickConfirm() {
      ...
      if (this.localTopicDialog.topicOperate === 1) {
        // 调用新增题目接口
        this.loading = true
        createTopic(this.createCveCreateTopicProto()).then(response => {
          this.loading = false
        ...
        // 调用更新题目接口
        this.loading = true
        updateTopic(this.createCveUpdateTopicProto()).then(response => {
          this.loading = false
          var data = response.data
          this.processData(data)
        })
    ...
    createCveCreateTopicProto() {
      var sysCveCreateTopic = proto.lookup('SysCveCreateTopic')
      var msg = sysCveCreateTopic.create({
      ...
      var encodeMsg = sysCveCreateTopic.encode(msg).finish()
      return encodeMsg.slice(0, encodeMsg.length)
    },
    // feat(QKSword)：生成更新题目protobuf
    createCveUpdateTopicProto() {
      var sysCveUpdateTopic = proto.lookup('SysCveUpdateTopic')
      var msg = sysCveUpdateTopic.create({
        title: this.localTopicInfo.title,
        desc: this.localTopicInfo.desc,
        time: this.localTopicInfo.time,
        overTime: this.localTopicInfo.overTime,
      ...
      var encodeMsg = sysCveUpdateTopic.encode(msg).finish()
      return encodeMsg.slice(0, encodeMsg.length)
    },
