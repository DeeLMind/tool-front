 ...
 * @Date: 2022-12-16 15:58:02
 * @LastEditors: QKSword 1064615639@qq.com
 * @LastEditTime: 2022-12-16 16:29:42
 * @FilePath: \lab-front\src\views\admin\components\OperateLabsTopic.vue
 * @Description: 操作labs题目组件
-->
<template>
  <el-dialog
...
<script>
import { createTopic, updateTopic } from '@/api/admin/sys-labs'
  ...
  methods: {
    ...
    clickConfirm() {
      ...
      if (this.localTopicDialog.topicOperate === 1) {
        // 调用新增题目接口
        this.loading = true
        createTopic(this.createLabsCreateTopicProto()).then(response => {
          this.loading = false
        ...
        // 调用更新题目接口
        this.loading = true
        updateTopic(this.createLabsUpdateTopicProto()).then(response => {
          this.loading = false
          var data = response.data
          this.processData(data)
        })
    ...
    createLabsCreateTopicProto() {
      var sysLabsCreateTopic = proto.lookup('SysLabsCreateTopic')
      var msg = sysLabsCreateTopic.create({
      ...
      var encodeMsg = sysLabsCreateTopic.encode(msg).finish()
      return encodeMsg.slice(0, encodeMsg.length)
    },
    // feat(QKSword)：生成更新题目protobuf
    createLabsUpdateTopicProto() {
      var sysLabsUpdateTopic = proto.lookup('SysLabsUpdateTopic')
      var msg = sysLabsUpdateTopic.create({
        title: this.localTopicInfo.title,
        desc: this.localTopicInfo.desc,
        time: this.localTopicInfo.time,
        overTime: this.localTopicInfo.overTime,
      ...
      var encodeMsg = sysLabsUpdateTopic.encode(msg).finish()
      return encodeMsg.slice(0, encodeMsg.length)
    },
